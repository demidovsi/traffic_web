...
class Session:
    ...
    def load(self):
                    ...
                    This method reads the session data from 'static/session.json' and
                    updates the session dictionary. It ensures that the loading process
                    is thread-safe by using a busy flag.

                    If the file does not exist or an error occurs during loading, the
                    session dictionary is set to an empty dictionary.

                    Raises:
                        Exception: If an error occurs during reading or parsing the JSON file.
                    """
                    while self.busy:
                        time.sleep(0.1)
                    self.busy = True
